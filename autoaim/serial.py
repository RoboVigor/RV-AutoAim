#!/usr/bin/env python
# -*- coding: utf-8 -*-
# CRC-8

import serial
import serial.tools.list_ports

port_list = list(serial.tools.list_ports.comports())
if len(port_list) >0:
    port_name = port_list[0].device

crc_list = []
crc_table = {}
for n in range(0, len(crc_list[:])):
    crc_table[n] = crc_list[n]


def crc_calculate(message, nBytes):
    remainder = 0x00
    data = 0
    byte = 0
    while byte < nBytes:
        data = message[byte] ^ remainder
        remainder = crc_table[data]
        byte = byte + 1
    return remainder

def send(data):
    with serial.Serial(port_name, 9600, timeout=0.1) as ser:
        ser.write(bytearray(data))
    #     x = ser.read(100)
    # print([i for i in x])
    # print(x)
    # return x

if __name__ == '__main__':
    test = crc_calculate([16, 0, 6], 3)
    print(test)
    send([87, 16, 0, 6, 0, 88])
