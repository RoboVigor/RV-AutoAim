...
def get_crc8_checksum(message):
    remainder = 0xff
    for byte in message:
        remainder = crc8_table[byte ^ remainder]
    return remainder
...
def get_crc16_checksum(message):
    remainder = 0xffff
    for byte in message:
        remainder = (remainder >> 8) ^ crc16_table[(byte ^ remainder) & 0x00ff]
    return remainder
...
def Unpacker():
    ...
    while True:
        ...
        # CRC8
        info['state'] = 'CRC8'
        crc8_checksum = get_crc8_checksum(info['packet'])
        info['packet'] += [byte]
        if crc8_checksum != byte:
            continue

        # Data
        info['state'] = 'Data'
        for i in range(info['len']+2):
            byte = yield info
            info['packet'] += [byte]
        info['id'] = info['packet'][5] + (info['packet'][6] << 8)

        # CRC16
        info['state'] = 'CRC16'
        crc16_checksum = get_crc16_checksum(info['packet'])
        byte1 = yield info
        info['packet'] += [byte1]
        byte2 = yield info
        info['packet'] += [byte2]
        if crc16_checksum == byte1 + (byte2 << 8):
            info['state'] = 'EOF'
