...
class AimImageToolbox():
    ...
    def threshold(self, draw=False):
        ...
        thresh = cv2.threshold(mat, (255-ret)*0.5+ret,
    ...
    def findLamps(self, draw=False, contours=None, rects=None, weights=None, passline=None):
        ...
        for i in range(0, len(rects)):
            merror = np.array(self.__getLampError(
                grayscales[i], rects[i], ellipses[i])).T
            mweights = np.array(weights)
            error = np.dot(merror, mweights)
            if error < passline:
                lamp = Lamp(contours[i], rects[i], ellipses[i],
                            grayscales[i], areas[i], error)
            ...
            if not pair_left.paired:
                for j in range(i+1, len(lamps)):
                    ...
                    if not _pair_right.paired:
                        # calc error
                        merror = np.array(self.__getPairError(
                            pair_left, _pair_right)).T
                        mweights = np.array(weights)
        ...
        if draw:
            ...
            for i in range(0, len(pairs)):
                ...
                cv2.rectangle(draw_mat, (left_lamp['x'], left_lamp['y']),
                              (right_lamp['x']+right_lamp['w'],
                               right_lamp['y']+right_lamp['h']),
                              (255, 255, 0)
                              )
        return pairs
...
class AimMat(AimImageToolbox):
    ...
    def __init__(self, img):
        ...
        self.lamps = self.findLamps(
            drawConfig[3], contours, rects, lamp_weights, lamp_passline)
        self.pairs = self.pairLamps(
            drawConfig[4], self.lamps, pair_weights, pair_passline)
    ...
    def centers(self):
        ...
        for pair in self.pairs:
            left = pair[0]
            right = pair[1]
            centers += [(left.x+(right.x+right.w-left.x)/2,
                         left.y+(right.y+right.h-left.y)/2)]
        return centers
...
def laiReal(callback=None):
    ...
    while True:
        ...
        print('  areas: ', autoaim.areas)
        print('  centers: ', autoaim.centers)
        if len(autoaim.centers) > 0:
            move = int((frame.shape[1]/2-autoaim.centers[0][0])*0.1)
            print('  move: ', move)
            if callback:
                callback(move)
        if cv2.waitKey(1) == 27:
