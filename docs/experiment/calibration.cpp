...
int main() {
    cout << "???";
    ifstream fin("../data/calibration.txt");
    ofstream fout("../data/caliberation_result.txt");
    ...
    while (getline(fin, filename)) {
        ...
        /* 提取角点 */
        count = findChessboardCorners(imageInput, board_size, image_points_buf);
        ...
        if (count == 0) {
            ...
            /* 亚像素精确化 */
            find4QuadCornerSubpix(view_gray, image_points_buf,
                                  Size(5, 5)); //对粗提取的角点进行精确化
            cornerSubPix(
                view_gray, image_points_buf, Size(5, 5), Size(-1, -1),
                TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
            image_points_seq.push_back(image_points_buf);
            //保存亚像素角点
            /* 在图像上显示角点位置 */
            drawChessboardCorners(view_gray, board_size, image_points_buf,
                                  false); //用于在图片中标记角点
    ...
    /*棋盘三维信息*/
    Size square_size =
        Size(19, 19); /* 实际测量得到的标定板上每个棋盘格的大小 */
    vector<vector<Point3f>> object_points; /* 保存标定板上角点的三维坐标
                                            */
    /*内外参数*/
    Mat cameraMatrix =
        Mat(3, 3, CV_32FC1, Scalar::all(0)); /* 摄像机内参数矩阵 */
    vector<int> point_counts;                // 每幅图像中角点的数量
    Mat distCoeffs =
        Mat(1, 5, CV_32FC1,
            Scalar::all(0)); /* 摄像机的5个畸变系数：k1,k2,p1,p2,k3 */
    ...
    *************************************************************************/
    ...
    for (int i = 0; i != image_count; i++) {
        ...
        StrStm.clear();
        imageFileName.clear();
        string filePath = "../data/calibration/img";
        StrStm << i + 1;
