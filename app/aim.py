...
def aim(img, serial=True, gui_update=False):
    ##### set up var #####
    global new_packet, ww, hh, config, predictor
    # config
...
config = autoaim.Config().read('default').data
predictor = autoaim.Predictor(config)
camera = autoaim.Camera(0, 'default').init((1280, 1024))
for i in range(1000):
    ...
    aim(camera.get_image()[1], serial=False)
